        ...
        for parquet_file in self.data_files:
            ...
            if parquet_file.endswith(".parquet"):
                try:
                    ...
                    # datasets lib wraps ArrowNotImplementedError in DatasetGenerationError,
                    # so we must check the full exception chain (str of e + all __cause__).
                    exc_chain_str = str(e)
                    cause = e.__cause__
                    while cause is not None:
                        exc_chain_str += " " + str(cause)
                        cause = cause.__cause__
                    if "Nested data" in exc_chain_str or "ArrowNotImplementedError" in exc_chain_str:
                        # Fallback for parquet files with nested structures (struct, list<struct>)
                        # that cause pyarrow ArrowNotImplementedError on chunked array outputs.
                        # Reading via iter_batches with a small batch_size avoids the issue.
                        logger.warning(
                            "datasets.load_dataset failed for %s due to nested structures, "
                            "falling back to pyarrow iter_batches: %s",
                            parquet_file,
                            e,
                        )
                        ...
                        batches = list(pf.iter_batches(batch_size=1024))
