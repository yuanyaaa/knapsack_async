...
| `rollout.total_rollout_steps`                                    | Total number of generated sequences across the whole run (sequence-level). |
...
* `rollout.total_rollout_steps`
  ...
  Sequence-level total budget: the total number of sequences to generate across the whole run.
  To align with the colocate setting, compute it by multiplying `train_batch_size`, `rollout.n` and `step`:
  `rollout.total_rollout_steps = data.train_batch_size * actor_rollout_ref.rollout.n * step`.
...
# SEQUENCE-level total budget: train_batch_size * rollout.n * step
...
* fully_async_policy
    * total_rollout_steps: 512*16*400  # sequence-level: train_batch_size * rollout.n * step
    * require_batches: 4
