...
class FullyAsyncTaskRunner:
    ...
    def run(self, config):
        if config.trainer.get("val_only", False):
            print("[ASYNC MAIN] Starting val_only mode (evaluate train data)...")
            self._run_val_only(config)
        else:
            print("[ASYNC MAIN] Starting fully async PPO training...")
            self._initialize_components(config)
            self._run_training_loop()

    def _run_val_only(self, config) -> None:
        """Run val_only mode: evaluate data and save annotated parquet.

        Set val_files to train_files in the shell script to evaluate train data.
        This mode only creates the trainer (with use_trainer_do_validate=True),
        runs _validate_process once, then exits. No rollouter or message queue needed.
        """
        ...
        # Force use_trainer_do_validate=True for val_only mode
        with open_dict(config):
            config.async_training.use_trainer_do_validate = True

        print("[ASYNC MAIN][val_only] Initializing model and tokenizer...")
        ...
        print("[ASYNC MAIN][val_only] Creating worker mapping and resource pools...")
        role_worker_mapping, ray_worker_group_cls = create_role_worker_mapping(config)
        self.components["role_worker_mapping"] = role_worker_mapping
        self.components["ray_worker_group_cls"] = ray_worker_group_cls

        # Only create trainer (no rollouter needed for val_only)
        print("[ASYNC MAIN][val_only] Creating FullyAsyncTrainer...")
        self._create_trainer(config)

        # Load checkpoint if available
        ray.get(self.components["trainer"].load_checkpoint.remote())

        # Run validation (which includes wake_up, _validate, sleep, and save annotated parquet)
        print("[ASYNC MAIN][val_only] Running validation...")
        ray.get(self.components["trainer"]._validate_process.remote())

        print("[ASYNC MAIN][val_only] Done!")
    ...
    def _create_rollouter(self, config) -> None:
        ...
        ray.get(rollouter.set_max_queue_size.remote())
...
def main(config):
    ...
    # In val_only mode, use_trainer_do_validate is forced to True internally
